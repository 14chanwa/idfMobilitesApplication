...
public class DataRetriever {
	...
	public static void main(String args[]) {
		...
		// This program requires you registering to Île-de-France Mobilités website in order to get
	...
	/**
	 * Need to provide an API key (access code linked to an Ile-de-France Mobilités Open Data
	 * account).
	 * @param _apiKey
	 */
	...
	/**
	 * Given a line ID and a stop ID, get the next times of the line at the stop according to the data of the API.
	 * @param _lineId
	 * @param _stopID
	 * @return
	 */
	public static List<Departure> getDeparturesLineAtStop(String _lineId, String _stopID) {
		...
		for (int i = 0; i < arr.length(); i++) {
			...
			if (arr.getJSONObject(i).getString("code").equals("duration")) {
				_departures.add(
						new Departure(
								arr.getJSONObject(i).getString("lineDirection"), 
	...
	/**
	 * Execute a HTTP/1.1 GET method using the provided URL, with the current CSRF token.
	 * @param url
	 * @return
	 */
